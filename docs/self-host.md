...
| `DataProtection__Directory` | `keys` beside the blob store | Where the ASP.NET key ring is written. It lives on the same volume as the blobs so it survives a container being recreated, and deliberately **beside** them rather than inside, because the reclaim sweep owns everything under the blob root. |
...
| `Oidc__RoleClaim` | `roles` | Claim HOPPER reads membership from. Pocket ID, Authentik and Keycloak publish groups as `groups`; pointing this at the wrong claim answers **403** to every admin request while the login itself succeeds. `Oidc__RoleClaim=groups` with `Oidc__AdminRole=<group>` maps a group you already have onto admin. **`Oidc__Scope` has to ask for it as well** - Pocket ID only emits the claim when `groups` is in the requested scope, so `openid profile email groups`. A claim nobody asked for is not in the token, and the refusal looks identical to a wrong claim name. |
| `Oidc__ValidAudiences__0` | `Oidc__ClientId` | Accepted `aud` values. Set when your issuer stamps something else. |
| `Oidc__FetchClaimsFromUserInfo` | `true` | When the access token carries no role claim, HOPPER reads the issuer's userinfo endpoint once per token and merges what it returns. Pocket ID, Okta and Entra keep group membership out of the access token, so without this their roles are unreachable no matter how `Oidc__RoleClaim` is set. An issuer that already puts roles in the token never triggers it. A userinfo endpoint that fails is logged and ignored rather than failing the request. |
...
Expected, and left alone on purpose. Encrypting the key ring at rest needs a certificate or a
platform key store, and there is nothing to protect: HOPPER authenticates with bearer tokens it does
not issue and holds no session or antiforgery state, so nothing is encrypted with these keys. They
also sit on the same volume as every mod jar - anyone who can read them can already read the jars.

The companion warning about keys not surviving the container is gone: they are written to the volume.
