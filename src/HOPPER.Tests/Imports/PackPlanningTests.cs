...
namespace HOPPER.Tests.Imports
{
    public class PackPlanningTests
    {
        ...
        public async Task ModrinthPlan_OverrideJars_AreIngested()
        {
            ...
            await Assert.That(sides["client-only.jar"]).IsEqualTo(ModSide.ClientOnly);
            await Assert.That(sides["server-only.jar"]).IsEqualTo(ModSide.ServerOnly);
        ...
        public async Task ModrinthPlan_ClientUnsupportedEntry_IsKeptAsServerOnly()
        {
            ...
            await Assert.That(sides.Keys.Order().ToList())
                .IsEquivalentTo(new[] { "no-env.jar", "server-side.jar" });
            await Assert.That(sides["server-side.jar"]).IsEqualTo(ModSide.ServerOnly);
            await Assert.That(sides["no-env.jar"]).IsEqualTo(ModSide.Both);
        }
