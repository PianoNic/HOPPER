        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                for (var offset = 0; offset < pending.Count && !stoppingToken.IsCancellationRequested; offset += BatchSize)
                {
                    ...
                    {
                        if (row.IconSha256 is not null)
                            continue;

                        var icon = await ModIconStore.FromJarAsync(blobs, row.Sha256, stoppingToken);
                        if (icon is null)
                            continue;
                ...
                if (filled > 0)
                    log.LogInformation("Backfilled icons for {Count} mod row(s).", filled);
            }
            ...
            {
                log.LogWarning(ex, "Mod icon backfill did not complete. It will be retried on the next start.");
            }
        }
    }
}
